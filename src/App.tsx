...
// Each entry: { label, mm }  (mm = decimal × 25.4)
const FRAC_OPTIONS = [
  { label: '3/16"', mm: (3/16)  * 25.4 },   // 4.76 mm
  { label: '1/4"',  mm: (1/4)   * 25.4 },   // 6.35 mm
  { label: '5/16"', mm: (5/16)  * 25.4 },   // 7.94 mm
  { label: '3/8"',  mm: (3/8)   * 25.4 },   // 9.53 mm
  { label: '7/16"', mm: (7/16)  * 25.4 },   // 11.11 mm
  { label: '1/2"',  mm: (1/2)   * 25.4 },   // 12.70 mm
  { label: '9/16"', mm: (9/16)  * 25.4 },   // 14.29 mm
  { label: '5/8"',  mm: (5/8)   * 25.4 },   // 15.88 mm
  { label: '3/4"',  mm: (3/4)   * 25.4 },   // 19.05 mm
];
...
function normalize(v, min, max) { return max === min ? 0 : (v - min) / (max - min); }
function beamDistance(input, beam, ranges) {
  const dH  = normalize(input.height,  ranges.height.min,  ranges.height.max)  - normalize(beam.height,  ranges.height.min,  ranges.height.max);
  const dF  = normalize(input.flange,  ranges.flange.min,  ranges.flange.max)  - normalize(beam.flange,  ranges.flange.min,  ranges.flange.max);
  const dW  = input.web     != null ? normalize(input.web,     ranges.web.min,     ranges.web.max)     - normalize(beam.web,     ranges.web.min,     ranges.web.max)     : 0;
  const dFT = input.flangeT != null ? normalize(input.flangeT, ranges.flangeT.min, ranges.flangeT.max) - normalize(beam.flangeT, ranges.flangeT.min, ranges.flangeT.max) : 0;
  return Math.sqrt(dH*dH*1.5 + dF*dF*1.2 + dW*dW*0.8 + dFT*dFT*0.8);
}
function findClosest(input, beams, top = 4) {
...
// ── Beam SVG ──────────────────────────────────────────────────────────────────
function BeamSVG({ heightRatio=0.5, flangeRatio=0.5, webRatio=0.4, flangeTRatio=0.4, size=160 }) {
  const W = size, H = size * 1.15;
  const fw = size*0.28 + flangeRatio*size*0.44;
  const bh = size*0.26 + heightRatio*size*0.50;
  const wt = Math.max(4, size*0.028 + webRatio*size*0.032);
  const ft = Math.max(5, size*0.032 + flangeTRatio*size*0.030);
  const cx = W/2, top = (H-bh)/2, bot = top+bh;
...
function FractionPicker({ label, selectedMM, onSelect, isMobile }) {
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", marginBottom:10, gap:6 }}>
        ...
        <span style={{ fontSize:9, fontWeight:500, letterSpacing:"0.07em", textTransform:"uppercase", background:"#f1f5f9", color:"#94a3b8", padding:"2px 6px", borderRadius:4 }}>
          opcional
        </span>
      </div>
...
export default function App() {
  ...
  // Unit for height & flange — default inches
  const [unit, setUnit]       = useState("in");

  // Height & flange: free-text inputs (cm or in)
  const [height,  setHeight]  = useState("");
  const [flange,  setFlange]  = useState("");

  // Thickness fields: stored directly as mm (selected from fraction pills)
  const [webMM,     setWebMM]     = useState(null);
  const [flangeTMM, setFlangeTMM] = useState(null);
  ...
  function handleUnitToggle(newUnit) {
    ...
    setUnit(newUnit);
    // thickness fields stay as mm — no conversion needed
  }

  const dv = (mmVal) => fromMM(mmVal, unit);
  const u  = unit; // "cm" or "in"
  const beamSize = isMobile ? 128 : 165;

  // Find the closest fraction label for display in results
  function fracLabel(mm) {
    if (mm == null) return "—";
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f7f8fa", fontFamily:"'DM Sans', -apple-system, sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <main style={{
        ...
        display:"flex", flexDirection:"column", gap: isMobile ? 22 : 36,
      }}>
        ...
        {/* ── CARD ── */}
        <div style={{
          background:"white", borderRadius: isMobile ? 18 : 22,
          border:"1px solid #e8ecf0",
          padding: isMobile ? "22px 18px 28px" : "36px 32px 42px",
          boxShadow:"0 2px 20px rgba(0,0,0,0.04)",
          display:"flex", flexDirection:"column", gap: isMobile ? 22 : 28,
        }}>

          {/* Unit toggle — affects height & flange only */}
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <div style={{ display:"flex", background:"#f1f5f9", borderRadius:10, padding:3, gap:2 }}>
              {["in","cm"].map(uu => (
                <button key={uu} onClick={() => handleUnitToggle(uu)} style={{
                  ...
                  minWidth: isMobile ? 50 : 44, minHeight: isMobile ? 40 : 34,
                }}>{uu}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 14 : 16 }}>
            {[
              { label:"Altura / Peralte (h)", val:height, set:setHeight, ph: u==="in" ? 'ej. 7.87"' : "ej. 20", hint: u==="in" ? '3.9" – 23.6"' : "10 – 60 cm" },
              { label:"Ancho de Patín (b)",   val:flange, set:setFlange, ph: u==="in" ? 'ej. 3.54"' : "ej. 9",  hint: u==="in" ? '2.0" – 8.7"'  : "5 – 22 cm"  },
              ...
              <div key={label}>
                ...
                <div style={{ position:"relative" }}>
                  <input
                    ...
                    onBlur={e   => e.target.style.borderColor = val ? "#0f172a" : "#e2e8f0"}
                  />
                  <span style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", fontSize:12, color:"#94a3b8", fontFamily:"'DM Mono', monospace", pointerEvents:"none" }}>
                    {u}
                  </span>
                </div>
          ...
          <div style={{ borderTop:"1px dashed #e8ecf0", paddingTop: isMobile ? 4 : 6 }}>
            <div style={{ fontSize:11, color:"#cbd5e1", letterSpacing:"0.07em", textTransform:"uppercase", marginBottom: isMobile ? 16 : 20 }}>
          ...
          {/* Search button */}
          <button className="sbtn" onClick={handleSearch} disabled={!canSearch} style={{
            width:"100%",
            padding: isMobile ? "18px" : "15px",
            background: canSearch ? "#0f172a" : "#e8ecf0",
            color: canSearch ? "white" : "#94a3b8",
            border:"none", borderRadius:14,
            fontSize: isMobile ? 16 : 15, fontWeight:500, letterSpacing:"0.02em",
        ...
        {searched && !loading && results.length > 0 && (
          <div>
            <div style={{ fontSize:11, fontWeight:500, color:"#94a3b8", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:12, paddingLeft:2 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {results.map(({ beam, dist }, i) => {
                ...
                const mColor = isTop
                  ...
                  : score > 90 ? "#22c55e" : score > 70 ? "#3b82f6" : score > 50 ? "#f59e0b" : "#94a3b8";
                ...
                return (
                  <div key={beam.id}>
                    ...
                    >
                      {/* Rank */}
                      <div style={{ width:30, height:30, borderRadius:8, flexShrink:0, background: isTop ? "rgba(255,255,255,0.1)" : "#f1f5f9", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:600, color: isTop ? "rgba(255,255,255,0.6)" : "#94a3b8" }}>
                        {i+1}
                      </div>
                      {/* Name + badge */}
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize: isMobile ? 17 : 19, fontWeight:500, color: isTop ? "white" : "#0f172a", fontFamily:"'DM Mono', monospace", letterSpacing:"-0.01em" }}>
                          {beam.name}
                        </div>
                      ...
                      <div style={{ textAlign:"right", flexShrink:0 }}>
                        ...
                        <div style={{ fontSize:11, color: isTop ? "rgba(255,255,255,0.3)" : "#94a3b8", marginTop:2 }}>{beam.weight} kg/m</div>
                      </div>
                      {/* Chevron */}
                    ...
                    {isExp && (
                      <div style={{
                        ...
                        <div style={{ fontSize:10, color:"#94a3b8", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:14 }}>
                        ...
                        <div style={{ display:"flex", flexDirection: isMobile ? "column" : "row", gap:10 }}>
                          <button style={{ flex:1, padding: isMobile ? "15px" : "13px", background:"#0f172a", color:"white", border:"none", borderRadius:10, fontSize:14, fontWeight:500, cursor:"pointer", minHeight: isMobile ? 50 : 44 }}>
                            Solicitar cotización
                          </button>
                          <button style={{ flex:1, padding: isMobile ? "15px" : "13px", background:"#f0fdf4", color:"#16a34a", border:"1.5px solid #bbf7d0", borderRadius:10, fontSize:14, fontWeight:500, cursor:"pointer", minHeight: isMobile ? 50 : 44, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
